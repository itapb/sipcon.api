...
namespace Data
{
    public class dService
    {
        ...
        public async Task<Models.Response<List<Models.ServiceFail>>> GetServiceFailToGenerate(string? filter, int? rowFrom, int userId,int dealerId, int? supplierId, DateTime? fromDate, DateTime? upToDate)
        {
            await _semaphore.WaitAsync(Util.Setting.TimeOut);
            try
            {
                return await _GetServiceFailToGenerate(filter, rowFrom, userId, dealerId, supplierId, fromDate, upToDate);
            }
            finally
            {
                _semaphore.Release();
        ...
        private async Task<Models.Response<List<Models.ServiceFail>>> _GetServiceFailToGenerate(string? filter, int? rowFrom, int userId,int dealerId, int? supplierId, DateTime? fromDate, DateTime? upToDate)
        {
            ...
            try
            {
                ...
                // Ejecución del SP
                Util.Data _data = Util.Data.GetInstance();
                DataTable _table = await _data.GetDataTable("USP_GET_MAINTENANCES_TOGENERATE", _parameter);
